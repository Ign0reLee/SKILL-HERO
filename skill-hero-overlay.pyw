...
heroes, proj, pops = [], [], []
enemy = {"hp":0,"hpMax":100,"alive":False,"hit":0,"dead":0,"x":772,"y":104,"cell":6}
...
# ---------- 레벨/진화 (영구 저장: ~/.claude/skill-hero-progress.json) ----------
PROG = os.environ.get("SKILL_HERO_PROG") or os.path.join(os.path.expanduser("~"), ".claude", "skill-hero-progress.json")
TIERCOL = ["#9aa6b2","#cd7f32","#d8dde6","#ffd700","#22d3ee","#f472b6","#a78bfa"]
confetti = []
def tierOf(l): return l // 10           # 10레벨마다 티어 증가
def load_prog():
    try:
        with open(PROG, encoding="utf-8") as f: d = json.load(f)
        return max(1, int(d.get("level", 1))), max(0, int(d.get("kills", 0)))
    except Exception:
        return 1, 0
level, kills = load_prog()
def save_prog():
    if os.environ.get("SKILL_HERO_NOSAVE"): return    # 셀프테스트/GIF 렌더 시 실제 진행도 보호
    try:
        os.makedirs(os.path.dirname(PROG), exist_ok=True)
        with open(PROG, "w", encoding="utf-8") as f: json.dump({"level": level, "kills": kills}, f)
    except Exception:
        pass
def milestone():                         # 10레벨마다: 빵빠레 + 진화
    global banner, bannerTxt
    banner = max(banner, 160); bannerTxt = "Lv.%d  ★ 진화 ★" % level
    cols = ["#ffd166","#46e0a0","#7c9cff","#f472b6","#fde047","#22d3ee"]
    for i in range(48):
        confetti.append({"x":random.random()*AW,"y":-random.random()*40,"vx":(random.random()-0.5)*2.4,
                         "vy":1+random.random()*2.2,"col":cols[i%6],"life":120})
def gainLevel():
    global level, kills
    kills += 1; level += 1; save_prog()
    if level % 10 == 0: milestone()
...
def setHeroes(n):
    ...
    del heroes[n:]; layout()
def spawnEnemy(hp=None):
    tr = tierOf(level); h = hp if hp else 100 + tr*40     # 티어마다 더 단단한 클로드(데모)
    enemy.update(hpMax=h, hp=h, alive=True, dead=0, hit=0, cell=6+min(tr,2)); proj.clear()
def lastBy(h, types):
    r = None
    for sid in h["eq"]:
...
def victory():
    ...
    if heroes:
        pops.append({"x":heroes[0]["x"]+20,"y":heroes[0]["y"]-20,"txt":"LEVEL UP! Lv.%d"%level,"col":"#ffd166","life":50,"vy":-0.6,"big":True})
def finish():
    for h in heroes:
...
def demo():
    ...
    if not dEntered:
        phase, label = p, l
        if onk == "spawn": setHeroes(3); spawnEnemy()
...
def update():
    ...
    for c in confetti: c["x"] += c["vx"]; c["y"] += c["vy"]; c["vy"] += 0.04; c["life"] -= 1
    confetti[:] = [c for c in confetti if c["life"] > 0 and c["y"] < AH+12]
...
def draw_claude(cv, dx=0, dy=0, cell=None, ghost=False):
    EC = cell if cell else enemy.get("cell", 6); g = CLAUDE_G; Wc = len(g[0]); Hc = len(g)
    wob = 0 if enemy["dead"] else round(math.sin(t/16)*1.5)
    ox = enemy["x"]+dx - Wc*EC//2; oy = enemy["y"]+dy - Hc*EC//2 + wob
...
def draw_hero(cv, h):
    ...
    if heroes and h is heroes[0]:   # 메인 용사: 레벨 + 진화 외견(왕관·테두리)
        tr = tierOf(level); tc = TIERCOL[min(tr, len(TIERCOL)-1)]
        if tr > 0:
            cv.create_rectangle(hx-3, hy-4, hx+8*HC+3, hy-2, fill=tc, width=0)
            cv.create_rectangle(hx-3, hy+12*HC+2, hx+8*HC+3, hy+12*HC+4, fill=tc, width=0)
            cv.create_rectangle(hx-3, hy-4, hx-1, hy+12*HC+4, fill=tc, width=0)
            cv.create_rectangle(hx+8*HC+1, hy-4, hx+8*HC+3, hy+12*HC+4, fill=tc, width=0)
...
def render_arena(cv):   # 이동 가능한 전장 창 (투명, 캐릭터만)
    cv.delete("all")
    if enemy["alive"]:
        tr = tierOf(level); EC = enemy.get("cell", 6)
        for k in range(min(tr, 2), 0, -1):                       # 뒤쪽 클로드 무리(흐린색)
            draw_claude(cv, -16*k, -8*k, max(3, EC-1-k), True)
        draw_claude(cv)
        if not enemy["dead"]:
            half = len(CLAUDE_G)*EC//2
            bw = 116; bx = enemy["x"] - bw//2; by = enemy["y"] - half - 16
            ...
            cv.create_text(enemy["x"], by-6, fill=COL["W"], font=("Consolas", 8),
                           text=("CLAUDE ×%d  " % (tr+1) if tr > 0 else "CLAUDE  ") + "%d/%d" % (enemy["hp"], enemy["hpMax"]))
            cv.create_text(enemy["x"], by+13, fill="#9fb2dd", font=("Consolas", 7), text="진화까지 %d킬" % (10-level % 10))
    elif phase != "idle":
    ...
    for c in confetti:
        cv.create_rectangle(c["x"], c["y"], c["x"]+3, c["y"]+3, fill=c["col"], width=0)
    if banner > 0:
        cv.create_text(enemy["x"], enemy["y"]-92, fill=("#ffd700" if "진화" in bannerTxt else "#46e0a0"),
                       font=("Consolas", 20, "bold"), text=bannerTxt)
